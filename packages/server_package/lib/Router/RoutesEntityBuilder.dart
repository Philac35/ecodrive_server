...
import 'package:shared_package/Controller/Controller_registry.dart';
...
class RouteEntityBuilder<T>{
  ...
  buildGetRoutes() {
    ...
    final classType = RuntimeType<T>()
        .toString(); //Use the package runtime_type to get the name of class of a generic
    ...
    for (var queryT in queryType.entries) {
      ...
      router.get('/${"$base/"}${classType}/${queryT.value}', (Request request,
          Response response) =>
      {

        if(controllerName != null)<T>{
          ret=controllerRegistry[controllerName]!().$(queryT.value)
        }
        else
          {
    ...
    buildPostRoutes() {
      ...
      for (var queryT in queryType) {
        //classType ~= T.className
        router.post('/${"$base/"}${classType}/${queryT}', (Request request,
            Response response)async
        {
         dynamic ret;
            var headers = <String, String>{"content-type": "application/json"};
          if(controllerName != null){
           controller= controllerRegistry[controllerName]!();

           // Extract JSON body
           final Map<String, dynamic> body = await extractJson(request);
          ret= controller.$(queryT)(body);
